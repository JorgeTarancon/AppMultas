-- Invitation acceptance and owner-only member management.
create or replace function public.invite_team_member(p_team_id uuid, p_email text, p_role public.team_role default 'editor')
returns table (invitation_id uuid, email text, role public.team_role, token text, expires_at timestamptz)
language plpgsql
security definer
...
begin
  if current_user_id is null or not public.has_team_role(p_team_id, array['owner']::public.team_role[]) then raise exception 'permission_denied'; end if;
  if normalized_email !~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' then raise exception 'email_invalid'; end if;
  if exists (select 1 from public.team_members member where member.team_id = p_team_id and lower((select email from auth.users where id = member.user_id)) = normalized_email) then raise exception 'already_member'; end if;
  insert into public.team_invitations (team_id, email, role, token_hash, invited_by, expires_at)
  values (p_team_id, normalized_email, p_role, encode(digest(raw_token, 'sha256'), 'hex'), current_user_id, now() + interval '7 days')
  ...
  return query select invitation.id, invitation.email, invitation.role, raw_token, invitation.expires_at;
end;
$$;
...
create or replace function public.change_team_member_role(p_team_id uuid, p_user_id uuid, p_role public.team_role)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if auth.uid() is null or not public.has_team_role(p_team_id, array['owner']::public.team_role[]) then raise exception 'permission_denied'; end if;
  if p_user_id = auth.uid() and p_role <> 'owner' then raise exception 'owner_cannot_demote_self'; end if;
  update public.team_members set role = p_role where team_id = p_team_id and user_id = p_user_id;
  if not found then raise exception 'member_not_found'; end if;
  insert into public.audit_log (team_id, user_id, action, entity_type, metadata)
  values (p_team_id, auth.uid(), 'role_change', 'team_member', jsonb_build_object('user_id', p_user_id, 'role', p_role));
end;
$$;
...
begin
  ...
  if not found then raise exception 'member_not_found'; end if;
  insert into public.audit_log (team_id, user_id, action, entity_type, metadata)
  values (p_team_id, auth.uid(), 'remove', 'team_member', jsonb_build_object('user_id', p_user_id));
end;
$$;
...
revoke execute on function public.change_team_member_role(uuid, uuid, public.team_role) from public;
...
grant execute on function public.change_team_member_role(uuid, uuid, public.team_role) to authenticated;
